...
import (
	...
	"github.com/open-edge-platform/image-composer/internal/utils/general/shell"
...
func PartitionTypeStrToUUID(partitionTypeStr string) (string, error) {
	if uuid, ok := partitionTypeNameToUUID[partitionTypeStr]; ok {
		return uuid, nil
	}
	return "", fmt.Errorf("partition type not found: %s", partitionTypeStr)
}

func PartitionUUIDToTypeStr(partitionUUID string) (string, error) {
	for k, v := range partitionTypeNameToUUID {
		if v == partitionUUID {
			return k, nil
		}
	}
	return "", fmt.Errorf("partition UUID not found: %s", partitionUUID)
}

func getPartedVersion() (int, int, error) {
	log := logger.Logger()
	output, err := shell.ExecCmd("parted --version", true, "", nil)
	if err != nil {
		return 0, 0, err
	}
	lines := strings.Split(output, "\n")
	if len(lines) > 0 && strings.Contains(lines[0], "parted") {
		...
		if len(versionParts) >= 2 {
			...
			log.Debugf(fmt.Sprintf("parted version: %d.%d", major, minor))
			return major, minor, nil
		}
	}
	return 0, 0, errors.New("could not parse parted version")
}

func isPartedSupportTypeCmds() bool {
	major, minor, _ := getPartedVersion()
	return major >= 4 || (major == 3 && minor >= 6)
}

func isPartedSupportEmptyString() bool {
	major, minor, _ := getPartedVersion()
...
func diskPartitionCreate(
	...
	if partitionTableType == "mbr" {
		...
		if name, ok := partitionInfo["Name"].(string); ok {
			partitionType = name
		} else if isPartedSupportEmptyString() {
			partitionType = "\"\""
		}
	}
	...
	if name, ok := partitionInfo["Name"].(string); ok {
		partitionName = name
	} else if id, ok := partitionInfo["ID"].(string); ok {
		partitionID = id
	...
	partitionFlagsList, _ := partitionInfo["Flags"].([]string)
	startSizeStr, _ := VerifyFileSize(partitionInfo["Start"])
	endSizeStr, _ := VerifyFileSize(partitionInfo["End"])
	fsType := partitionInfo["FsType"].(string)
	if slice.Contains(fsTypeList, fsType) {
		return "", fmt.Errorf("unknown fs type: %s", fsType)
	}
	partitionTypeStr, _ := partitionInfo["Type"].(string)
	partitionTypeUUID, _ := partitionInfo["TypeUUID"].(string)
	...
	var endSector int
	if end, ok := partitionInfo["End"].(int); ok && end == 0 {
		endSector = 0
	} else {
	...
	var cmdStr string
	if endSector == 0 {
		cmdStr = fmt.Sprintf("flock --timeout 5 %s parted %s --script mkpart %s %s %s 100%%",
			diskPath, diskPath, partitionType, fsType, startSectorStr)
	} else {
		cmdStr = fmt.Sprintf("flock --timeout 5 %s parted %s --script mkpart %s %s %s %s",
			diskPath, diskPath, partitionType, fsType, startSectorStr, endSectorStr)
	}
	...
	cmdStr = fmt.Sprintf("flock --timeout 5 %s partprobe -s %s", diskPath, diskPath)
	_, err = shell.ExecCmd(cmdStr, true, "", nil)
	if err != nil {
	...
	if partitionTableType == "gpt" && isPartedSupportTypeCmds() {
		typeUUID := partitionTypeUUID
		if typeUUID == "" && partitionTypeStr != "" {
			typeUUID, _ = PartitionTypeStrToUUID(partitionTypeStr)
		}
		if typeUUID != "" {
			cmdStr = fmt.Sprintf("flock --timeout 5 %s parted %s --script type %d %s",
				diskPath, diskPath, partitionNum, typeUUID)
			_, err := shell.ExecCmd(cmdStr, true, "", nil)
			if err != nil {
				return "", fmt.Errorf("failed to set partition type for partition %d: %v", partitionNum, err)
			}
		}
	}

	// Set partition flags
	partitionFlagsMap := map[string]string{
		"esp":       "esp",
		"grub":      "bios_grub",
		"bios_grub": "bios_grub",
		"bios-grub": "bios_grub",
	...
	for _, flag := range partitionFlagsList {
		if mapped, ok := partitionFlagsMap[flag]; ok && mapped != "" {
			cmdStr = fmt.Sprintf("flock --timeout 5 %s parted %s --script set %d %s on",
				diskPath, diskPath, partitionNum, mapped)
			_, err := shell.ExecCmd(cmdStr, true, "", nil)
			if err != nil {
				return "", fmt.Errorf("failed to set partition flag %s for partition %d: %v", flag, partitionNum, err)
			}
		}
	}
	...
	_, err = shell.ExecCmd(cmdStr, true, "", nil)
	if err != nil {
		return "", fmt.Errorf("failed to refresh partition table")
	}
	...
	if fsType == "fat32" || fsType == "fat16" || fsType == "vfat" {
		cmdStr = fmt.Sprintf("mkfs -t vfat %s", diskPartDev)
		_, err := shell.ExecCmd(cmdStr, true, "", nil)
		if err != nil {
			return "", fmt.Errorf("failed to format partition %d with fs type %s: %v", partitionNum, fsType, err)
		}
	} else if fsType == "ext2" || fsType == "ext3" || fsType == "ext4" || fsType == "xfs" {
		...
		if additionalFlags != "" {
			cmdStr = fmt.Sprintf("mkfs -t %s %s %s", fsType, additionalFlags, diskPartDev)
		} else {
			cmdStr = fmt.Sprintf("mkfs -t %s %s", fsType, diskPartDev)
		}
		_, err := shell.ExecCmd(cmdStr, true, "", nil)
		if err != nil {
			return "", fmt.Errorf("failed to format partition %d with fs type %s: %v", partitionNum, fsType, err)
...
func diskPartitionDelete(diskPath string, partitionNum int) error {
	...
	cmdStr := fmt.Sprintf("flock --timeout 5 %s parted %s --script rm %d", diskPath, diskPath, partitionNum)
	_, err := shell.ExecCmd(cmdStr, true, "", nil)
	if err != nil {
...
func DiskPartitionsCreate(diskPath string, partitionsList []map[string]interface{}, partitionTableType string) (map[string]string, error) {
	partIDDiskDevMap := make(map[string]string)
	log := logger.Logger()
	if partitionTableType == "gpt" {
		cmdStr := fmt.Sprintf("flock --timeout 5 %s parted %s --script mklabel gpt", diskPath, diskPath)
		_, err := shell.ExecCmd(cmdStr, true, "", nil)
		if err != nil {
		...
		for i, partitionInfo := range partitionsList {
			...
			partIDDiskDevMap[partitionInfo["ID"].(string)] = diskPartDev
		}
	} else if partitionTableType == "mbr" {
		var partitionType string
		var partitionNum int
		maxPrimaryPartitionsNum := 4
		cmdStr := fmt.Sprintf("flock --timeout 5 %s parted %s --script mklabel msdos", diskPath, diskPath)
		_, err := shell.ExecCmd(cmdStr, true, "", nil)
		if err != nil {
