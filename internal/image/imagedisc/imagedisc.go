...
var IsPartedSupportTypeCmds bool
var IsPartedSupportEmptyString bool
...
func checkPartedFeatureSupport() error {
	major, minor, err := getPartedVersion()
	if err != nil {
		return fmt.Errorf("failed to get parted version: %v", err)
	}
	IsPartedSupportTypeCmds = major >= 4 || (major == 3 && minor >= 6)
	IsPartedSupportEmptyString = major >= 4 || (major == 3 && minor >= 5)
	return nil
}
...
func diskPartitionCreate(
	...
	if endSector == 0 {
		cmdStr = fmt.Sprintf("flock --timeout 5 %s parted %s --script mkpart %s %s %s 100%%",
			diskPath, diskPath, partitionType, partitionInfo.FsType, startSectorStr)
	} else {
		cmdStr = fmt.Sprintf("flock --timeout 5 %s parted %s --script mkpart %s %s %s %s",
			diskPath, diskPath, partitionType, partitionInfo.FsType, startSectorStr, endSectorStr)
	}
	...
	if partitionTableType == "gpt" && IsPartedSupportTypeCmds {
		...
		if typeGUID != "" {
			cmdStr = fmt.Sprintf("flock --timeout 5 %s parted %s --script type %d %s",
				diskPath, diskPath, partitionNum, typeGUID)
			_, err := shell.ExecCmd(cmdStr, true, "", nil)
			if err != nil {
...
func DiskPartitionsCreate(diskPath string, partitionsList []config.PartitionInfo, partitionTableType string) (map[string]string, error) {
	...
	log := logger.Logger()
	err := checkPartedFeatureSupport()
	if err != nil {
		return nil, fmt.Errorf("failed to check parted feature support: %v", err)
	}
