...
build_emt3_raw_image() {
  ...
  set +e
  output=$( sudo -S ./os-image-composer build image-templates/emt3-aarch64-minimal-raw.yml 2>&1)
  build_exit_code=$?
  set -e
  ...
  if [ $build_exit_code -eq 0 ] && echo "$output" | grep -q "image build completed successfully"; then
    ...
    if [ "$RUN_QEMU_TESTS" = true ]; then
      echo "Running QEMU boot test for ARM EMT3 raw image..."
      if run_qemu_boot_test "emt3-aarch64-minimal"; then
        echo "QEMU boot test PASSED for ARM EMT3 raw image"
      else
        echo "QEMU boot test FAILED for ARM EMT3 raw image"
        exit 1
      fi
